...
import {initializeUnityAds, maybeShowInterstitial} from './lib/unityAds';
import {
  getAnalytics,
  getCrashlytics,
  ...
  useEffect(() => {
    ...
    initializeUnityAds()
      .then(initialized => {
        ...
        // Unity Ads has no separate Android App-Open format in this integration,
        // so use a single interstitial after the app has settled.
        timer = setTimeout(() => {
          maybeShowInterstitial().catch(error =>
            console.warn('[UnityAds] App-open ad failed:', error),
          );
        }, 4500);
      })
      .catch(error =>
        console.warn('[UnityAds] Startup initialization failed:', error),
      );
