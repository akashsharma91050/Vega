...
import {showInterstitial} from '../../lib/ads/unityAds';
...
const Player = ({route}: Props): React.JSX.Element => {
  ...
  // Gate actual playback behind a single Unity Ads interstitial per player
  // session. `adGateOpen` starts false so the video source stays empty
  // until the ad finishes (or fails to load), then we let it through and
  // never show another interstitial for this screen instance.
  ...
  useEffect(() => {
    ...
    showInterstitial().finally(() => setAdGateOpen(true));
  }, []);
  ...
  const videoPlayerProps = useMemo(
    () => ({
      ...
      source: {
        ...
        uri: adGateOpen ? processedStreamUrl || '' : '',
        bufferConfig: {
