...
import {maybeShowInterstitial} from '../../lib/unityAds';
...
const Player = ({ route }: Props): React.JSX.Element => {
  ...
  const playerOpenAdShownRef = useRef(false);
  const midrollShowingRef = useRef(false);
  const lastMidrollAtRef = useRef(0);
  const MIDROLL_INTERVAL_SECONDS = 10 * 60;
  ...
  useEffect(() => {
    if (playerOpenAdShownRef.current || remoteMediaClient || streamLoading || !processedStreamUrl) {
      return;
    }

    let cancelled = false;
    const timer = setTimeout(() => {
      if (cancelled || playerOpenAdShownRef.current) return;
      playerOpenAdShownRef.current = true;
      maybeShowInterstitial().catch(error =>
        console.warn('[UnityAds] Player-open ad failed:', error),
      );
    }, 2500);
  ...
  }, [processedStreamUrl, remoteMediaClient, streamLoading]);

  useEffect(() => {
    if (remoteMediaClient || streamLoading || !processedStreamUrl) return;
    if (currentPlaybackTime < MIDROLL_INTERVAL_SECONDS) return;
    if (midrollShowingRef.current) return;
    if (currentPlaybackTime - lastMidrollAtRef.current < MIDROLL_INTERVAL_SECONDS) return;

    // Never interrupt the final 60 seconds of a video.
    const duration = videoPositionRef.current.duration;
    if (duration > 0 && duration - currentPlaybackTime < 60) return;

    midrollShowingRef.current = true;
    playerRef.current?.pause();

    maybeShowInterstitial()
      .catch(error => {
        console.warn('[UnityAds] Mid-roll ad failed:', error);
        return false;
      })
      .finally(() => {
        lastMidrollAtRef.current = currentPlaybackTime;
        midrollShowingRef.current = false;
        // Resume whether the ad was shown or not; this keeps a not-ready ad
        // from accidentally leaving playback paused.
        playerRef.current?.resume();
      });
  }, [currentPlaybackTime, processedStreamUrl, remoteMediaClient, streamLoading]);
